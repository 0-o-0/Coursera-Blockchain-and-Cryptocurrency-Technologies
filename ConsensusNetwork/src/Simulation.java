...
public class Simulation {
   ...
   public static void main(String[] args) {
      ...
      HashMap<Set<Transaction>, Integer> countCompliantNodes = new HashMap<Set<Transaction>, Integer>();
      HashMap<Set<Transaction>, Integer> countMaliciousNodes = new HashMap<Set<Transaction>, Integer>();
      ...
      for (int i = 0; i < numNodes; i++) {
         ...
         System.out.println();
         System.out.println();
         if (!countCompliantNodes.containsKey(transactions)) {
             countCompliantNodes.put(transactions, 0);
             countMaliciousNodes.put(transactions, 0);
         }
         if (nodes[i] instanceof CompliantNode) {
             countCompliantNodes.put(transactions, countCompliantNodes.get(transactions) + 1);
             totalCompliant++;
         } else {
             countMaliciousNodes.put(transactions, countMaliciousNodes.get(transactions) + 1);
         }
      }
      System.out.println("Total compliant nodes: " + totalCompliant);
      Iterator it = countCompliantNodes.entrySet().iterator();
      while (it.hasNext()) {
          Map.Entry pair = (Map.Entry)it.next();
          Set<Transaction> txs = (Set<Transaction>)pair.getKey();
          int countCompliant = (int)pair.getValue();
          int countMalicous = countMaliciousNodes.get(txs);
          System.out.println("Tx count: " + txs.size() + " Compliant nodes: " + countCompliant + " Malicious nodes: " + countMalicous);
      }
